...
**Obecny etap:** Play Store prep — Batch 1 (manifest/config/strings) zrobiony. **Regresja:** Setup wizard „Allow background activity" nie działa po usunięciu `REQUEST_IGNORE_BATTERY_OPTIMIZATIONS` (Task 2.11) → Continue szary, hard block setupu. Decyzja: **Opcja A** (użyj `ACTION_IGNORE_BATTERY_OPTIMIZATION_SETTINGS` + Toast hint). Fix odłożony — patrz `docs/future_polish_fixes.md` #27. Plan: `docs/play-store/02_implementation_plan.md`.
...
| **High** | Fix regresji Setup wizard — battery button (Opcja A, `future_polish_fixes.md` #27) | OTWARTE, decyzja A podjęta (04-20), impl odłożony — blokuje smoke test Batch 1 |
