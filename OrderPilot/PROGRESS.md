...
**Obecny etap:** Rename CourierAssist → OrderPilot zakończony. Parser false positives + Bolt watch mode. Czeka na build + test.
**Aktywne branche:** `fix-formaty` (rename + parser false positives + Bolt watch mode), `fix/locale-audit` (audyt lokalizacyjny), `feature/multi-overlay` (tip development)
...
| Data | Zmiana |
|------|--------|
| 04-04 | **Rename CourierAssist → OrderPilot** — package `com.orderpilot.app`, klasy (OrderPilotApp, OrderPilotAccessibilityService), moduł `OrderPilot/`, SharedPrefs key, log tagi CA_→OP_, strings.xml (3 locale), themes.xml, docs, memory files |
| 04-03 | **Parser false positives** — usunięty regex `z` z CUR, fix backtracking AMOUNT_FALLBACK_REGEX, guard historyScreenMarkers w UberOcrParser, normalizeOcrDigits() |
