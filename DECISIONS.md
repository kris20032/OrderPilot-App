...
## D9 — OrderPilot ZAMROŻONY produktowo: nie promujemy, nie rozwijamy, nie kasujemy
- **Status:** ✅ Aktywna
- **Data decyzji:** 2026-08-31
- **Decyzja:** kończymy inwestowanie czasu w OrderPilota. Apka zostaje w Google Play w stanie, w jakim jest (v1.1.0), bez promocji, bez rozwoju i bez planu monetyzacji. Utrzymanie ograniczone do jednej rzeczy: raz w roku podbicie `targetSdk` pod wymóg Google (komenda + `tools/wydaj.py`, ~godzina). Nie usuwamy jej ze sklepu.
- **Dlaczego:** pomiar realnego użycia z 31.08.2026 pokazał **~1 aktywny użytkownik dziennie przy 13 instalacjach**, a po 21.08 zero otwarć — czyli install base to rodzina i płatni testerzy, nie kurierzy. Do tego: (1) brak skonfigurowanego konta sprzedawcy, więc nawet sukces daje 0 zł bez dobudowania płatności; (2) sufit rynku niski — kurierzy słabo płacą za narzędzia, sensowny przychód wymaga tysięcy pobrań; (3) produkt czyta cudze ekrany (Uber/Glovo/Bolt), więc każda zmiana ich UI psuje serce apki = stały podatek od czasu; (4) godzina Krzysztofa włożona w telefony Impulseo ma dziś nieporównywalnie wyższą oczekiwaną wartość (1500-3000 zł za stronę po jednym telefonie) niż godzina promocji darmowej apki.
- **Konsekwencje:** `docs/promo/PLAN_ROZWOJU_DEEP_RESEARCH_2026-06-28.md` zostaje jako materiał na półce — NIE jest już „następnym krokiem". Backlog (`docs/future_polish_fixes.md`, audyt 2026-06-28, M14/M17) zamrożony. Apka pełni odtąd rolę **wizytówki sprzedażowej Impulseo** („mamy własną aplikację w Google Play") — to jedyna wartość, jaką dziś generuje, i nie wymaga żadnej pracy.
- **Co odwróciłoby tę decyzję:** pojawienie się kanału dystrybucji za darmo (np. ktoś ze społeczności kurierów sam zaczyna ją polecać) ALBO klient gotowy zapłacić za wersję pod siebie. Sam pomysł „zareklamujmy ją" — nie wystarczy.
- **Powiązania:** `todo.md` (Stan teraz), pamięć `project_orderpilot_growth_plan`, `reference_play_api_wydawanie` (jak wydać przy corocznym podbiciu).
